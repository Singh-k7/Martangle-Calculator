...
function initTheme(){
    ...
    if(btn){
        btn.addEventListener('click', ()=>{
            ...
            function resetTodayStats() { state.todayProfit = 0; state.todayLoss = 0; saveTodayStats(); updateTodayUI(); }
            ...
            function recalc() {
                ...
                let totalProfit = 0, totalLoss = 0, wins = 0, losses = 0, tradesDone = 0, maxStake = 0;
                ...
                for (let i = 0; i < state.rows.length; i++) {
                    ...
                    let lossSum = 0; for (let j = 0; j <= i; j++) { if (state.rows[j].result === 'loss') lossSum += Number(state.rows[j].stake || 0); }
                    r.lossSoFar = lossSum;
          ...
          tbl.innerHTML = `<thead><tr><th>Trade #</th><th>Stake</th><th>Profit Target (editable)</th><th>Profit if Win</th><th>Loss So Far</th><th>Capital Before</th><th>Result</th><th>Action</th></tr></thead><tbody></tbody>`;
                ...
                state.rows.forEach((r, i) => {
                    ...
                    tr.innerHTML = `
        <td>${r.trade}</td>
        <td class="${over}">${fmt(r.stake)}</td>
        <td><input type="number" class="small pt" data-idx="${i}" value="${toFixed(r.profitTarget)}"></td>
        <td>${fmt(r.profitIfWin)}</td>
        <td>${fmt(r.lossSoFar)}</td>
        <td>${fmt(r.capitalBefore)}</td>
        <td>${r.result ? (r.result === 'win' ? '<span class="win">WIN</span>' : '<span class="loss">LOSS</span>') : '-'}</td>
        <td>
          <button class="winBtn" data-idx="${i}" ${!r.canPlace && r.result === '' ? 'disabled' : ''}>Win</button>
          <button class="lossBtn gray" data-idx="${i}" ${!r.canPlace && r.result === '' ? 'disabled' : ''}>Loss</button>
        </td>`;
                    tbody.appendChild(tr);
                });

                // attach listeners
                tbody.querySelectorAll('.pt').forEach(inp => inp.addEventListener('input', (e) => {
                    const idx = Number(e.target.dataset.idx);
                    state.rows[idx].profitTarget = parseFloat(e.target.value) || 0;
                    recalc();
                }));

                tbody.querySelectorAll('.winBtn').forEach(b => b.addEventListener('click', (e) => {
                    const idx = Number(e.target.dataset.idx);
                    // if can't place, ignore
                    if (!state.rows[idx].canPlace) { alert('Not enough capital for this stake.'); return; }
                    state.rows[idx].result = 'win';
                    recalc();
                }));

                tbody.querySelectorAll('.lossBtn').forEach(b => b.addEventListener('click', (e) => {
                    const idx = Number(e.target.dataset.idx);
                    if (!state.rows[idx].canPlace) { alert('Not enough capital for this stake.'); return; }
            ...
            $('simulateLossChain').addEventListener('click', () => { state.rows.forEach(r => r.result = 'loss'); recalc(); });
            $('resetAll').addEventListener('click', () => { state.rows.forEach(r => r.result = ''); recalc(); });
            $('exportCSV').addEventListener('click', () => {
                let csv = 'Trade,Stake,ProfitTarget,ProfitIfWin,LossSoFar,CapitalBefore,Result\n';
                state.rows.forEach(r => csv += `${r.trade},${r.stake},${r.profitTarget},${r.profitIfWin},${r.lossSoFar},${r.capitalBefore},${r.result}\n`);
                const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'binary_trades.csv'; a.click(); URL.revokeObjectURL(url);
            });
            $('resetStats').addEventListener('click', () => { if (confirm('Reset today\'s saved profit/loss?')) { resetTodayStats(); } });

            // live update when inputs change
            ['totalCapital', 'startAmount', 'desiredProfit', 'payoutPercent', 'maxTrades'].forEach(id => $(id).addEventListener('input', () => {
                // if maxTrades changed -> rebuild rows
                if (id === 'maxTrades') {
                    initRows();
                } else {
                    // update state values and recalc
                    state.initialCapital = parseFloat($('totalCapital').value) || state.initialCapital;
                    state.startAmount = parseFloat($('startAmount').value) || state.startAmount;
                    state.desiredProfit = parseFloat($('desiredProfit').value) || state.desiredProfit;
                    state.payoutPercent = parseFloat($('payoutPercent').value) || state.payoutPercent;
